  ...
  return (
    <div style={{ maxWidth: '470px', margin: '0 auto', width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '8px 12px', gap: '4px' }}>
        ...
        <button onClick={() => { if (!requireLogin()) return; setNotified(!notified); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', borderRadius: '50%', display: 'flex', color: notified ? '#3b82f6' : 'var(--text-secondary)', transition: 'all 0.2s' }}>
      ...
      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', padding: '16px', gap: '8px' }}>
        {[
