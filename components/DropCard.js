...
export default function DropCard({ drop, index = 0 }) {
  ...
  const likes = drop.engagement?.likes || drop.likes || 0;
    ...
    >
      ...
      <Link href={`/drop/${drop.id}`} style={{ textDecoration: 'none' }}>
        <div style={{ position: 'relative', width: '100%', aspectRatio: '1', overflow: 'hidden', background: '#080808' }}>
          ...
          />
          {/* Countdown + Notify Me overlay at bottom */}
          <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '10px 12px 4px', gap: '2px' }}>
        ...
        {/* Comment */}
        <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', borderRadius: '50%', display: 'flex', transition: 'all 0.2s' }}
          onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.15)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; }}
        >
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', borderRadius: '50%', display: 'flex', transition: 'all 0.2s' }}
          onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.15)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; }}
        >
      ...
      <div style={{ padding: '0 16px 6px', display: 'flex', gap: '14px', fontSize: '13px' }}>
        <span style={{ fontWeight: 700, color: '#fff' }}>{formatNumber(likes)} likes</span>
        <span style={{ color: '#737373' }}>{formatNumber(saves)} saves</span>
        <span style={{ color: '#737373' }}>{formatNumber(comments)} comments</span>
      </div>
