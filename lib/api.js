...
export async function fetchFollowingDrops(category) {
  const params = category && category !== 'all' ? `?category=${category}` : '';
  return apiFetch(`/api/drops/following${params}`);
}
...
// Toggle email notification for a drop
export async function toggleNotify(dropId) {
  return apiFetch(`/api/drops/${dropId}/notify`, { method: 'PUT' });
}

// Check if user is notified for a drop
export async function checkNotify(dropId) {
  return apiFetch(`/api/drops/${dropId}/notify`);
}

// Delete a drop (brand owner only)
export async function deleteDrop(dropId) {
  return apiFetch(`/api/drops/${dropId}`, { method: 'DELETE' });
}
...
export async function signup(email, name, password, role = 'user', brandCategory = null) {
  return apiFetch('/api/auth/signup', {
    method: 'POST',
    body: JSON.stringify({ email, name, password, role, brandCategory }),
  });
}
